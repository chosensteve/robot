 ...
 * STRICT BINARY THRESHOLDS VERSION
 */
...
void readSensors() {
  for (int i = 0; i < 16; i++) {
    ...
    // Very short delay allowing mux signal to settle
    delayMicroseconds(10); 
    
    // Read the 12-bit analog signal
    int rawValue = analogRead(muxSig); 
    
    // Apply user thresholds (BINARY APPROACH):
    ...
    if (rawValue >= 2800 && rawValue <= 4000) { 
      // Arbitrary binary "ON" value
      sensorValues[i] = 1000; 
    } else {
      // Arbitrary binary "OFF" value
      sensorValues[i] = 0;
    }
  }
}
